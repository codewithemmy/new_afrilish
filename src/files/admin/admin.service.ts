...
export default class AdminService {
  ...
  static async resetPasswordService(adminPayload: {
    email: string
    oldPassword: string
    newPassword: string
  }) {
    const { email, oldPassword, newPassword } = adminPayload

    const validate = await AdminRepository.fetchAdminWithPassword({ email })

    if (!validate) return { success: false, msg: adminMessages.INVALID }

    const checkPassword = await verifyPassword(oldPassword, validate.password)

    if (!checkPassword)
      return { success: false, msg: adminMessages.PASSWORD_ERROR }

    const hashedPassword = await hashPassword(`${newPassword}`)
    const updatePartner = await AdminRepository.updateAdminDetails(
      { email },
      { password: hashedPassword },
    )

    if (!updatePartner)
      return { success: false, msg: `Unable to reset password` }
